...
def _dump_terrain_cam_to_wandb(env, num_envs: int) -> None:
    ...
    wandb.log({"terrain/sensor_rgb_env0": wandb.Image(rgb.cpu().numpy())})
    print(f"[INFO] Logged terrain sensor frame to wandb (shape={tuple(rgb.shape)}).")


@hydra_run_config(run_config_name=args_cli.run_config_name)
def main(run_cfg: RunConfig):
    ...
    # Dump one sensor frame to wandb BEFORE wrapping so scene access is uncluttered.
    if not log_cfg.no_wandb:
        _dump_terrain_cam_to_wandb(env, env_cfg.num_envs)
